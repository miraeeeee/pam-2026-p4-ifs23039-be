...
fun Application.configureRouting() {
    ...
    val sdgsService: SdgsService by inject()
    ...
    routing {
        get("/") {
            call.respondText("API telah berjalan. Dibuat oleh Raynal - IFS23039.")
        }
        ...
        route("/sdgs") {
            get {
                sdgsService.getAllSdgs(call)
            }
            post {
                sdgsService.createSdgs(call)
            }
            get("/{id}") {
                sdgsService.getSdgsById(call)
            }
            put("/{id}") {
                sdgsService.updateSdgs(call)
            }
            delete("/{id}") {
                sdgsService.deleteSdgs(call)
            }
            get("/{id}/image") {
                sdgsService.getSdgsImage(call)
            }
        }
